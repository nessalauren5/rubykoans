...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_character_classes_give_options_for_a_character
    ...
    assert_equal ["cat", "bat", "rat"], animals.select { |item| item[/[cbr]at/] }
  end
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    assert_equal nil, "start end"[/\Aend/] #end is not at the beginning of the string
  end

  def test_slash_z_anchors_to_the_end_of_the_string
    assert_equal "end", "start end"[/end\z/]
    assert_equal nil, "start end"[/start\z/] #start is not at the end of the string
  end
  ...
  def test_parentheses_also_capture_matched_content_by_number
    assert_equal "Gray", "Gray, James"[/(\w+), (\w+)/, 1]  # match first word
    assert_equal "James", "Gray, James"[/(\w+), (\w+)/, 2] # match second word
  end
  ...
  def test_sub_is_like_find_and_replace
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] } #take first matched word (beginning with t) and replace it with the first character
  end

  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] } #take first matched word (beginning with t) and replace ALL with the first character
  end
end
