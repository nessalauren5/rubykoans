...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    result = [2, 3, 4].inject(0) { |sum, item| sum + item }#sum=0
    assert_equal 9, result

    result2 = [2, 3, 4].inject(1) { |product, item| product * item } #product=1
    assert_equal 24, result2

    # Extra Credit:
    # Describe in your own words what inject does.
    # inject accepts an argument as the initial value on which a block function is performed.
  end
  ...
  #   # code to read 'file'
  # The first way only executes the do block if the file is opened, the latter executes regardless if opening the file fails
